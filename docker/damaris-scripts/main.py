...
def blocking_task(client):
    ...
    while True:
        ...
        count += 1
        sample = simulation_data[2]
        doc.add_next_tick_callback(partial(
            stream_update, x=simulation_data[0], y=simulation_data[1], sim_string=sample))
...
try:
    ...
    print('ERROR launching Bokeh server: Could not get the Dask scheduler file argument ' +
          str(sys.argv[0:],) + ' [ -s | --scheduler-file ] <schedulerfile.json> ')
    sys.exit(2)
for opt, arg in opts:
    if opt == '-h':
        print(
            'INFO: to launch the Bokeh server, specify the Dask scheduler file as an argument: >bokeh serve view_app --port 5100 --args [ -s | --scheduler-file ] <schedulerfile.json> ')
        sys.exit()
...
l = p.circle(x='x', y='y', color=linear_cmap(
    'sim_string', palate,  1.0, float(num_sims)), size=5, source=source)
